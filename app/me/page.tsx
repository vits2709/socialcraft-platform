...
// ─── Personalizzazione profilo — costanti ─────────────────────────────────────
...
export default function MePage() {
  ...
  const [profileShowcaseBadges, setProfileShowcaseBadges] = useState<string[]>([]);

  // stati di salvataggio granulari
  ...
  async function loadAll(silent = false) {
    ...
    try {
      ...
      if (meJson?.ok) {
        ...
        setProfileShowcaseBadges(Array.isArray(u.showcase_badges) ? u.showcase_badges : []);
      }
  ...
  async function saveAvatar(emoji: string) {
    ...
    const json = await patchProfile({ avatar_emoji: emoji }).catch(() => ({ ok: false }));
    setSavingAvatar(false);
    if (json.ok) { setSavedAvatar(true); setTimeout(() => setSavedAvatar(false), 2000); }
  }
  ...
  async function saveColor(color: string) {
    ...
    setSavingColor(true);
    setSavedColor(false);
    const json = await patchProfile({ profile_color: color }).catch(() => ({ ok: false }));
    setSavingColor(false);
    if (json.ok) { setSavedColor(true); setTimeout(() => setSavedColor(false), 2000); }
  }

  async function saveVisibility(pub: boolean) {
    setProfilePublic(pub);
    await patchProfile({ is_public: pub }).catch(() => {});
  }
  ...
  async function saveBio() {
    ...
    setSavingBio(true);
    setSavedBio(false);
    const json = await patchProfile({ bio: profileBio.trim() || null }).catch(() => ({ ok: false, error: "Errore di rete" }));
    setSavingBio(false);
    if (!json.ok) { setBioErr(json.error === "bio_too_long" ? "Bio troppo lunga (max 100 caratteri)" : json.error ?? "Errore"); return; }
    setSavedBio(true);
  ...
  async function saveShowcase() {
    ...
    const json = await patchProfile({ showcase_badges: profileShowcaseBadges }).catch(() => ({ ok: false }));
    setSavingShowcase(false);
    if (json.ok) { setSavedShowcase(true); setTimeout(() => setSavedShowcase(false), 2000); }
  }

  async function saveSocial() {
    setSavingSocial(true);
    setSavedSocial(false);
    const json = await patchProfile({
      instagram: profileInstagram.trim() || null,
      tiktok: profileTiktok.trim() || null,
      twitter_x: profileTwitterX.trim() || null,
    }).catch(() => ({ ok: false }));
    setSavingSocial(false);
    if (json.ok) { setSavedSocial(true); setTimeout(() => setSavedSocial(false), 2000); }
  }
  ...
  async function saveUsername() {
    ...
    const json = await patchProfile({ username: profileUsername.trim() }).catch(() => ({ ok: false, error: "Errore di rete" }));
  ...
  function toggleShowcaseBadge(badgeId: string) {
    setProfileShowcaseBadges((prev) => {
      if (prev.includes(badgeId)) return prev.filter((id) => id !== badgeId);
      if (prev.length >= 3) return [...prev.slice(1), badgeId]; // rimuove il primo, aggiunge il nuovo
      return [...prev, badgeId];
    });
  }
...
return (
  ...
  >
      ...
      <div
        style={{
          borderRadius: 18,
          border: "1px solid rgba(45,27,105,0.13)",
          background: "white",
      ...
      >
        ...
        >
          <div>
            <div style={{ fontWeight: 950, fontSize: 17 }}>✏️ Personalizza il tuo profilo</div>
            <div style={{ fontSize: 13, opacity: 0.6, marginTop: 2 }}>
              Scegli come appari agli altri esploratori
            </div>
          </div>
          {profileUsername && (
            <a
        ...
        <div style={{ display: "grid", gap: 0 }}>
          ...
          >
            <div>
              <div style={{ fontWeight: 800, fontSize: 14 }}>
                {profilePublic ? "🌍 Profilo pubblico" : "🔒 Profilo privato"}
              </div>
              <div style={{ fontSize: 12, opacity: 0.55, marginTop: 2 }}>
                {profilePublic
                  ? "Il tuo profilo è visitabile dagli altri utenti"
                  : "Il tuo profilo non è visitabile da altri utenti"}
              </div>
            </div>
            <button
              type="button"
              onClick={() => saveVisibility(!profilePublic)}
              style={{
                position: "relative",
                width: 48,
                height: 26,
              ...
              aria-label={profilePublic ? "Rendi privato" : "Rendi pubblico"}
            >
              <span
                style={{
                  position: "absolute",
                  top: 3,
                  left: profilePublic ? 24 : 3,
                  width: 20,
                  height: 20,
                  borderRadius: "50%",
                  background: "white",
          ...
          >
            <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 10 }}>🔗 Username</div>
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <div style={{ position: "relative", flex: 1, minWidth: 160 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setProfileUsername(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))
                  }
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    border: `1px solid ${usernameErr ? "#ef4444" : "rgba(0,0,0,0.14)"}`,
                    fontSize: 14,
                    fontWeight: 700,
                    boxSizing: "border-box",
                    fontFamily: "inherit",
              ...
              <button
                ...
                style={{ fontSize: 13, whiteSpace: "nowrap" }}
              >
                {savingUsername ? "..." : savedUsername ? "✅ Salvato!" : "Salva"}
              </button>
            </div>
            {usernameErr && (
              <div style={{ fontSize: 12, color: "#ef4444", marginTop: 6 }}>{usernameErr}</div>
            )}
          </div>
            ...
            >
              ...
              <div
                style={{
                  ...
                  fontSize: 28,
                  flexShrink: 0,
                  boxShadow: `0 3px 12px ${profileColor}55`,
                  transition: "background 0.2s",
                }}
              >
                {profileAvatar || "🧭"}
              ...
              <div>
                <div style={{ fontWeight: 800, fontSize: 14 }}>😎 Avatar</div>
                <div style={{ fontSize: 12, opacity: 0.55, marginTop: 2 }}>
                  {savingAvatar ? "Salvataggio..." : savedAvatar ? "✅ Salvato!" : "Clicca su un'emoji per selezionarla"}
                </div>
              </div>
            </div>
              ...
              <div key={cat.label} style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {cat.emojis.map((emoji) => {
                    const selected = profileAvatar === emoji;
                    return (
                      <button
                        key={emoji}
                        type="button"
                        ...
                        style={{
                          ...
                          background: selected ? "rgba(45,27,105,0.09)" : "rgba(0,0,0,0.03)",
                          fontSize: 20,
                          cursor: "pointer",
                          display: "flex",
                          alignItems: "center",
                          ...
                          boxShadow: selected ? "0 0 0 3px rgba(45,27,105,0.15)" : "none",
                        }}
                        title={emoji}
                      >
                        {emoji}
          ...
          >
            <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 12 }}>🎨 Colore profilo</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {PRESET_COLORS.map(({ hex, label }) => {
                const selected = profileColor.toLowerCase() === hex.toLowerCase();
                return (
                  <button
                    key={hex}
                    type="button"
                    ...
                    style={{
                      ...
                      boxShadow: selected ? `0 0 0 2px ${hex}` : "0 1px 4px rgba(0,0,0,0.15)",
                    }}
                  >
                    {selected && (
                      <span style={{ color: "white", fontWeight: 900, fontSize: 16, lineHeight: 1 }}>
                        ✓
                      </span>
                    )}
                  </button>
            ...
            {(savingColor || savedColor) && (
              <div style={{ fontSize: 12, opacity: 0.6, marginTop: 8 }}>
                {savingColor ? "Salvataggio..." : "✅ Salvato!"}
              </div>
            )}
          </div>
          ...
          >
            <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 10 }}>
              📝 Bio{" "}
              <span style={{ fontWeight: 400, opacity: 0.45, fontSize: 12 }}>
            ...
            />
            {bioErr && (
              <div style={{ fontSize: 12, color: "#ef4444", marginTop: 4 }}>{bioErr}</div>
            )}
            <button
              type="button"
              onClick={saveBio}
            ...
            >
              {savingBio ? "Salvataggio..." : savedBio ? "✅ Salvato!" : "Salva bio"}
            </button>
          </div>
          ...
          >
            <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 4 }}>🏆 Badge vetrina</div>
            ...
            {sbloccati.length === 0 ? (
              ...
              <>
                    ...
                    {profileShowcaseBadges.map((bid) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span style={{ fontWeight: 800, color: rc.color, fontSize: 12 }}>{b.name}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                >
                  {sbloccati.map(({ def }) => {
                    const rc = RARITY_CFG[def.rarity];
                    const isSelected = profileShowcaseBadges.includes(def.id);
                    return (
                      <button
                        key={def.id}
                        type="button"
                        onClick={() => toggleShowcaseBadge(def.id)}
                        style={{
                          padding: "10px 8px",
                          borderRadius: 10,
                          border: isSelected
                            ? "2px solid #2D1B69"
                            : "2px solid rgba(0,0,0,0.08)",
                          background: isSelected ? "rgba(45,27,105,0.07)" : "rgba(0,0,0,0.02)",
                          cursor: "pointer",
                          textAlign: "center",
                          position: "relative",
                          transition: "border 0.15s, background 0.15s",
                ...
                >
                  {savingShowcase ? "Salvataggio..." : savedShowcase ? "✅ Salvato!" : "Salva vetrina"}
                </button>
              </>
            )}
          </div>
          ...
          <div style={{ padding: "16px 18px" }}>
            <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 12 }}>🔗 Link social</div>
                ...
                [
                  {
                    ...
                    setter: setProfileInstagram,
                  },
                  {
                    label: "🎵 TikTok",
                    placeholder: "@tuousername",
                    value: profileTiktok,
                    setter: setProfileTiktok,
                  },
                  {
                    label: "𝕏 X (Twitter)",
                    placeholder: "@tuousername",
                    value: profileTwitterX,
                    setter: setProfileTwitterX,
                  },
                ] as { label: string; placeholder: string; value: string; setter: (v: string) => void }[]
            ...
            >
              {savingSocial ? "Salvataggio..." : savedSocial ? "✅ Salvato!" : "Salva social"}
            </button>
          </div>
